...
document.addEventListener('DOMContentLoaded', () => {
  ...
  --------------------------------------------------- */
  const WHATSAPP_NUMBER = '2349138852810'; // 
  ...
  /* ---------------------------------------------------
     5) TESTIMONIAL MARQUEE — duplicate the track content
        once so the CSS loop (translateX -50%) is seamless
  --------------------------------------------------- */
  const track = document.getElementById('testimonialTrack');
  if (track) {
    const clone = track.innerHTML;
    track.insertAdjacentHTML('beforeend', clone);
    track.querySelectorAll('blockquote').forEach((card, i) => {
      if (i >= track.children.length / 2) card.setAttribute('aria-hidden', 'true');
    });
  }
